...
2. **约束限制 (Set Boundaries)**：在 Prompt 或 `.cursorrules` 中明确指定技术栈，例如："使用 Vue3 + Vite + TailwindCSS 搭建"。
...
<div class="p-5 rounded-xl bg-slate-50 border border-slate-200 shadow-sm relative overflow-hidden">
  ...
  <p class="text-[13px] text-slate-600 leading-relaxed">Prompt 技巧再高超，如果 AI 没有拿到业务级、架构级的真实语境也是白搭。<br/><br/>构建 <strong>AGENTS.md</strong> 或 <strong>.cursorrules</strong>，挂载 <strong>MCP 数据源</strong>，做好上下文工程，才是团队规模化使用工具的底座。</p>
</div>
