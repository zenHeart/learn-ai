...
<!-- Removed duplicate demo text, keep VibeExample in its designated place -->
...
1. **计划 (plan)** 明确需求，制定计划
2. **检查 (check)** 确保 AI 理解意图
3. **执行 (execute)** 让 AI 完成任务
4. **验证 (verify)** 验收结果，循环此流程
...
- 应用 `.cursorignore` 实现业务隔离与合规
- 开启商业隐私保护策略切断代码训练后路
- 利用团队配额共享解决 API 调用速率限制
