...
3. **Session/Chat** 大模型本身是无状态的，AI 工具通过 Session/Chat 存储对话上下文(一般是 markdown 文件或 json 对象)

<iframe
  src="https://gpt-tokenizer.dev/"
  class="w-full h-100 border-none rounded-lg shadow-lg mt-2"
...
1. **Token** 大模型处理信息的最小文本单元 <VibeExample id="2.1" />
2. **Context Window (上下文窗口)**：大模型能处理的最大 Token 数量
    <img src="/context-window.svg" alt="上下文窗口" class="h-70 mx-auto" />
3. **[Embeddings(嵌入)](https://developers.google.com/mace-learning/crash-course/embeddings/embedding-space?hl=zh-cn)** 将文本/图片/音频/视频转换为向量数据的技术 <VibeExample id="2.2" />
4. **[Retrieval Augmented Generation(RAG)](https://arxiv.org/abs/2005.11401v4)** 结合嵌入与大模型，从外部数据源检索信息，帮助模型生成更准确结果的技术, 延伸阅读 [How Cursor Indexes Codebases Fast](https://read.engineerscodex.com/p/how-cursor-indexes-codebases-fast)
...
1. **[Transformer 架构](https://arxiv.org/abs/1706.03762)** 通过**注意力机制**让每个词"关注"其他词，从而理解语义，[交互示意图](https://bbycroft.net/llm) <VibeExample id="2.3" />
...
2. **mode** 模型执行的不同的模式，典型的模式包括
   1. **plan** 模型不会做任何代码修改，而是会深入思考先输出计划
   2. **edit** 模型会做代码修改，并执行
   3. **yolo|solo|headless** 用户只需给出任务模型会自行循环知道完成任务
