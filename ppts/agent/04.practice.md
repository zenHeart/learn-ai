...
<strong>核心优势</strong>: 整个逻辑就是一段原生的 <code>while</code> 循环和 <code>if-else</code>。你可以随时 <code>console.log</code>，随时打断点调试。没有任何魔法，这就是最稳定、最工业级的 Agent 实现方式。
</div>
...
1. **Code-as-Agent**: 用代码控制流程，而非依赖黑盒框架
2. **Coder + Reviewer**: 分离执行与验证，防止死循环
3. **状态机思维**: 清晰的流转逻辑，可观测、可调试
