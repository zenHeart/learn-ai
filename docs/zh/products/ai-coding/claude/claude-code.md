...
会话控制：
  ...
  -r, --resume [session]     恢复历史会话（可选指定名称）
      ...
      --effort <level>       推理深度：low / medium / high / xhigh
      --permission-mode <mode> 权限模式：normal / auto / plan / auto-edit
...
| `Shift+Tab` | 切换权限模式（Normal → Auto → Plan → Auto-Accept） |
| `Option+P` / `Alt+P` | 切换模型 |
| `↑ / ↓` | 导航命令历史 |
| `Ctrl+R` | 反向搜索历史 |
...
用 `Shift+Tab` 循环切换 Normal / Auto / Plan / Accept Edits / Auto-Accept 五种模式，或在 `settings.json` 中设置默认值：
...
- [claude-code-setup](https://github.com/vercel-labs/claude-code-setup) — Anthropic 官方推荐的自动化配置插件，一键安装社区最佳实践（Hooks + MCP + Skills）
...
> - [claude-code-setup](https://github.com/vercel-labs/claude-code-setup) — Anthropic 官方推荐的自动化配置插件，一键安装 Hooks/MCP/Skills 最佳实践
