...
本章节为不同目标的学习者提供清晰的路线图。

### 路线一：快速上手（1天）
适合：只想快速使用 OpenClaw 的用户
...
```json5
{
  session: {
    maintenance: {
      mode: "enforce",      // 维护模式：enforce (强制执行), warn (警告)
...
| 英文 | 中文 | 说明 |
|------|------|------|
| Gateway | 网关 | OpenClaw 核心进程，负责消息路由和会话管理 |
| Agent | 智能体 | AI 助手的运行时实例 |
| Session | 会话 | 对话上下文，包含历史消息和状态 |
| Channel | 通道 | 消息接入方式（如飞书、Discord） |
| Subagent | 子智能体 | 任务分解机制，并行处理复杂任务 |
| Skill | 技能 | 扩展智能体能力的模块 |
| Hook | 钩子 | 生命周期事件拦截机制 |
| MCP | 模型上下文协议 | 扩展工具能力的标准协议 |
| ACP | Agent 通信协议 | 多智能体间通信协议 |
...
**Q: Node.js 版本不兼容怎么办？**
A: OpenClaw 推荐 Node 24，Node 22 LTS 也支持。使用 `nvm` 切换版本。

**Q: 安装脚本失败怎么解决？**
A: 手动安装：`npm install -g openclaw@latest`
...
2. 查看 `openclaw logs` 获取详细错误
3. 确认防火墙/网络设置

**Q: 如何开启调试模式？**
A: `openclaw gateway --verbose`

### 使用问题

**Q: 子智能体是什么？如何使用？**
A: 子智能体允许任务分解并行处理。详见「核心概念 - 子智能体」。

**Q: 技能和插件有什么区别？**
A: 技能是工作区级别的扩展，插件是系统级别的扩展。详见「技能」章节。

### 安全问题

**Q: 如何防止未授权访问？**
A:
1. 配置 `allowFrom` 限制访问来源
2. 开启 `auth` 认证
3. 使用沙盒隔离

**Q: 提示注入攻击如何防护？**
A: 详见「安全配置」章节，包括输入验证和输出过滤。

## 11. 扩展指南（进阶）

已为你补充以下专项文档：

- **[CLI 命令参考](./cli.md)** - 所有 CLI 命令完整索引，按功能模块组织
- **[部署指南](./deployment.md)** - Docker / PM2 / systemd / LaunchAgent 多场景部署
- **[安全配置指南](./security.md)** - 访问控制、工具安全、提示注入防护、夜间审计
- **[技能开发指南](./skills.md)** - 从零开发自定义技能，扩展智能体能力
