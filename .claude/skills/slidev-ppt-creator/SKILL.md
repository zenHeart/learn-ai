...
- **可见 vs 隐藏内容**：`::: tip`、普通段落、任何不在 `<!-- -->` 里的文字，观众翻页都会看到。时长、跳过、口播调度必须完整放进 `<!-- 演讲者备注：... -->`。案例见 [lessons-learned.md](assets/lessons-learned.md)。
- **组件配置与渲染分离**：文案和可配置项放进 `<script setup>` 的具名数据，`<template>` 只负责布局。
- **Tailwind 类名禁止插值拼接**：`` :class="`border-blue-${n}`" `` 不会生效。每条数据预置完整字面量 `cls`。
- **布局 (Layouts)**:
...
| [lessons-learned.md](assets/lessons-learned.md) | 踩坑：生造词标题、讲者备注泄漏、术语不一致、断章引用、Tailwind 动态类名、密度过冲、事实核查 |