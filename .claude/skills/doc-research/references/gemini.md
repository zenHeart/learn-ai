...
- 轴 B（受众复杂度）：`code-assist` 官方开场即 "organizations with strict data security and compliance requirements"、VPC-SC、IP indemnification，是**企业级强信号**，因此即使它的概念依赖很浅（IDE 插件），也要往后压；`google-pro` 同理——订阅层级本身概念独立（轴 A 允许很前），但它回答的是"我该为团队掏哪一档钱"，属于付费决策受众，压到最后。
- `canvas` 轴 A 零依赖、轴 B 最轻（Gemini App 内即可用），但它不在工程化主线上，所以放在"更多产品"组首位而非核心组。

## 监控页面（What's New 驱动更新的最小子集）

- Gemini CLI 发布通道与变更：<https://geminicli.com/docs/changelogs/>
